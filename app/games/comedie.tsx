/**
 * performance.tsx — Concours Performance — 4 onglets
 *
 * UPLOAD SÉCURISÉ :
...
async function getValidToken(): Promise<{
  ...
  try {
    ...
    if (!res.ok) { console.warn('[Performance] Rafraîchissement token échoué'); return null; }
    ...
    console.log('[Performance] Token rafraîchi avec succès');
    return { uid, token: data.access_token, refresh_token: data.refresh_token, expires_at: data.expires_at };
  } catch (e) {
    console.warn('[Performance] getValidToken error:', e);
    return null;
  }
}
...
// ─── Composant principal ──────────────────────────────────────────────────────
export default function Performance({ userId: userIdProp, onBack, onClose }: PerformanceProps) {
    ...
    <SafeAreaView style={styles.root}>
      ...
      <LinearGradient colors={['#2A1500', '#0C0A08']} style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSub}>Concours de performance</Text>
        </View>
