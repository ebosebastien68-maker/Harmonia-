/**
 * music.tsx — Concours Music — 4 onglets
 *
 * UPLOAD SÉCURISÉ :
 ...
 *   4. Frontend confirme la soumission au backend (submitAudio avec path + duration_s)
...
async function getValidToken(): Promise<{
  ...
  try {
    ...
    if (!res.ok) { console.warn('[Music] Rafraîchissement token échoué'); return null; }
...
interface AudioSlot {
  index: number;
  pickedAudio: { uri: string; ext: string; duration_s?: number } | null;
  description: string;
...
// ─── Composant principal ──────────────────────────────────────────────────────
export default function Music({ userId: userIdProp, onBack, onClose }: MusicProps) {
  ...
  const [slots,          setSlots]          = useState<AudioSlot[]>([]);
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    const res = await fetch(`${BACKEND_URL}/music`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
        user_id:       uid,
  ...
  // ─── Choisir un audio pour un slot ──────────────────────────────────────
  const pickVideoForSlot = useCallback(async (slotIndex: number) => {
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: ['audio/mpeg', 'audio/wav', 'audio/x-wav', 'audio/mp4',
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const uri   = asset.uri;
        const raw   = asset.name.split('.').pop()?.toLowerCase() || 'mp3';
        const ext   = ['mp3', 'wav', 'm4a', 'aac', 'ogg'].includes(raw) ? raw : 'mp3';
        haptic.light();
        setSlots(prev => prev.map(s =>
          s.index === slotIndex
            ? { ...s, pickedAudio: { uri, ext, duration_s: undefined } }
            : s
        ));
      }
    } catch (err) {
      console.warn('[Music] pickAudio error:', err);
    }
  }, []);

  const clearSlotVideo = useCallback((slotIndex: number) => {
    setSlots(prev => prev.map(s =>
      s.index === slotIndex ? { ...s, pickedAudio: null } : s
    ));
  }, []);
  ...
  const slotsReady    = slotsToUpload.filter(s => !!s.pickedAudio);
  const allSlotsReady = slotsToUpload.length > 0 && slotsReady.length === slotsToUpload.length;
  const totalSlots    = slots.length;
  const filledSlots   = slots.filter(s => s.submitted || s.pickedAudio).length;
  ...
  const handleSubmitAll = useCallback(async () => {
    ...
    const pending = slotsToUpload.filter(s => !!s.pickedAudio);
    ...
    try {
      for (let i = 0; i < pending.length; i++) {
        ...
        await api({
          ...
          duration_s:  slot.pickedAudio!.duration_s ?? null,
        });
      }
      ...
      setSlots(prev => prev.map(s => ({ ...s, pickedAudio: null, description: '' })));
      setSubmitRun(null);
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <LinearGradient colors={['#1E0A3C', '#080810']} style={styles.header}>
        {(onBack || onClose) && (
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSub}>Concours de music</Text>
        </View>
      ...
      {loading ? (
        ...
        <ScrollView style={styles.flex} contentContainerStyle={styles.scroll}>
          ...
          {tab === 'mine' && !selSession && (
            mySessions.length === 0
              ? <Empty icon="musical-notes-outline" text="Aucune session rejointe" hint="Explorez les concours disponibles" />
              : mySessions.map(s => (
                    ...
                    <View style={styles.cardBody}>
                      ...
                      {s.current_run && (
                        <View style={styles.infoRow}>
                          <Ionicons name="musical-notes-outline" size={13} color={C.muted} />
                      ...
                      .map(s => (
                        ...
                        >
                          <View style={styles.cardBody}>
                            ...
                            <View style={styles.infoRow}>
                              <Ionicons name="musical-notes-outline" size={13} color={C.submissions} />
...
function AudioSlotCard({
  ...
  const hasPicked = !!slot.pickedAudio;
          ...
          {hasPicked ? (
            <>
              ...
              <View style={styles.slotPickedVideoContent}>
                ...
                {slot.pickedAudio!.duration_s != null && (
                  <View style={styles.slotDurationBadge}>
                    <Ionicons name="time-outline" size={11} color={C.cream} />
                    <Text style={styles.slotDurationText}>{formatDuration(slot.pickedAudio!.duration_s)}</Text>
                  </View>
                )}
                <Text style={styles.slotVideoExt}>.{slot.pickedAudio!.ext.toUpperCase()}</Text>
              </View>
              {!uploading && (
                <TouchableOpacity
            ...
            <View style={styles.slotEmptyContent}>
              ...
              <Text style={styles.slotEmptyHint}>MP3 · WAV · M4A</Text>
            </View>
          )}
        </TouchableOpacity>
      )}
  ...
  return (
    <View style={styles.quotaBar}>
      <View style={styles.quotaLabelRow}>
        <Ionicons name="musical-notes-outline" size={13} color={done ? C.finished : C.muted} />
...
function RunStatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { bg: string; text: string; label: string }> = {
    draft:            { bg: '#1E0A3C', text: C.purpleLight,  label: '📝 Brouillon'           },
    submissions_open: { bg: '#0A2A22', text: C.submissions, label: '🎥 Soumissions ouvertes' },
    voting_open:      { bg: '#1E0A3C', text: C.voting,      label: '🗳️ Votes ouverts'       },
