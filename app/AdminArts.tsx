/**
 * AdminArts.tsx — Interface d'administration Arts
 *
 * VUES :
 *   1. Liste des sessions → créer / modifier / supprimer / voir joueurs
 *   2. Détail session    → liste des runs + créer run
 *   3. Détail run        → joueurs inscrits + soumissions + classement + changer status
 */
...
interface Submission {
  id: string; user_id: string; nom_complet: string;
  image_url: string; description?: string; votes: number;
}
...
interface AdminArtsProps {
  adminEmail:    string;
  adminPassword: string;
  onBack:        () => void;
}

export default function AdminArts({ adminEmail, adminPassword, onBack }: AdminArtsProps) {
  ...
  const [runTab,      setRunTab]      = useState<'players' | 'submissions' | 'classement'>('submissions');
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    const res = await fetch(`${BACKEND_URL}/admin-arts`, {
      method:  'POST',
  ...
  const setRunStatus = async (runId: string, status: string) => {
    ...
    const doIt = async () => {
      ...
      try {
        ...
        if (selSession) await loadRuns(selSession.id);
        if (selRun?.id === runId) {
          setSelRun(prev => prev ? { ...prev, status: status as any } : prev);
        }
      } catch (e: any) { setError(e.message); }
      finally { setActionLoading(null); }
    };
  ...
  const deleteSubmission = async (subId: string) => {
    const msg = 'Supprimer cette soumission ?';
    const doIt = async () => {
    ...
    <View style={styles.root}>
      ...
      <LinearGradient colors={['#5B21B6', '#7C3AED']} style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>🎨 Admin Arts</Text>
      ...
      {loading ? (
        <View style={styles.centered}><ActivityIndicator size="large" color={C.purple} /></View>
      ) : (
            ...
            <>
              ...
              {showSessionForm && (
                <View style={styles.formBox}>
                  ...
                  <TextInput style={styles.input} value={formTitle} onChangeText={setFormTitle} placeholder="Ex: Concours Dessin #1" />
                  ...
                  <View style={styles.switchRow}>
                    ...
                    <Switch value={formIsPaid} onValueChange={setFormIsPaid} trackColor={{ true: C.purple }} />
                  </View>
                  {formIsPaid && (
                    <>
              ...
              {sessions.length === 0 ? (
                <View style={styles.empty}><Ionicons name="easel-outline" size={40} color={C.muted} /><Text style={styles.emptyText}>Aucune session Arts</Text></View>
              ) : (
                sessions.map(s => (
                  <View key={s.id} style={styles.card}>
                    ...
                    <View style={styles.cardActions}>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => openSession(s)}>
                        <Ionicons name="grid" size={16} color={C.purple} /><Text style={[styles.actionText, { color: C.purple }]}>Gérer</Text>
                      </TouchableOpacity>
          ...
          {view === 'runs' && selSession && (
            <>
              <View style={styles.infoBar}>
                <Ionicons name="people-outline" size={15} color={C.purple} />
              ...
              {showRunForm && (
                <View style={styles.formBox}>
                  <Text style={styles.formTitle}>🖼️ Nouveau Run</Text>
                  <Text style={styles.label}>Titre *</Text>
                  <TextInput style={styles.input} value={runTitle} onChangeText={setRunTitle} placeholder="Ex: Manche 1 — Dessin libre" />
                  <Text style={styles.label}>Max soumissions par joueur</Text>
                ...
                runs.map(run => (
                  <View key={run.id} style={styles.runCard}>
                    ...
                    <View style={styles.runMeta}>
                      <MetaCell icon="images-outline"  label="Max sub" value={String(run.max_submissions)} />
                      <MetaCell icon="people-outline"  label="Joueurs" value={String(run.players_count)} />
                      <MetaCell icon="grid-outline"    label="Soum."   value={String(run.submissions_count)} />
                    </View>
                    ...
                    <View style={styles.runActions}>
                      ...
                      <TouchableOpacity style={styles.viewBtn} onPress={() => openRun(run)}>
                        <Ionicons name="eye-outline" size={16} color={C.purple} />
                ...
                {(['submissions', 'classement'] as const).map(t => (
                  ...
                  >
                    <Text style={[styles.innerTabText, runTab === t && styles.innerTabTextActive]}>
                      {t === 'submissions' ? '🖼️ Soumissions' : '🏆 Classement'}
                    </Text>
                  </TouchableOpacity>
                ))}
              </View>
              ...
              {runTab === 'submissions' && (
                <>
                  {submissions.length === 0 ? (
                    <View style={styles.empty}><Ionicons name="images-outline" size={40} color={C.muted} /><Text style={styles.emptyText}>Aucune soumission</Text></View>
                  ) : (
                    submissions.map(sub => (
                      <View key={sub.id} style={styles.subCard}>
                        <View style={styles.subCardLeft}>
                          <Text style={styles.subAuthor}>{sub.nom_complet}</Text>
                          {sub.description ? <Text style={styles.subDesc} numberOfLines={2}>{sub.description}</Text> : null}
                          <View style={styles.subVotes}>
                            <Ionicons name="heart" size={14} color={C.danger} />
                            <Text style={styles.subVotesText}>{sub.votes} votes</Text>
                          </View>
                        </View>
                        <TouchableOpacity style={styles.subDelBtn} onPress={() => deleteSubmission(sub.id)} disabled={!!actionLoading}>
...
function RunStatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { bg: string; text: string; label: string }> = {
    draft:            { bg: '#EDE9FE', text: C.draft,        label: '📝 Brouillon'         },
    submissions_open: { bg: '#CCFBF1', text: C.submissions,  label: '🖼️ Soumissions'       },
...
const styles = StyleSheet.create({
  ...
  addBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
                gap: 8, backgroundColor: C.purple, borderRadius: 12, paddingVertical: 14, marginBottom: 16 },
  ...
  saveBtn:     { flex: 1, padding: 12, borderRadius: 10, alignItems: 'center', backgroundColor: C.purple },
  ...
  enrollBtn:    { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: C.purpleLight,
  ...
  viewBtn:      { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#EDE9FE',
                  borderRadius: 10, paddingVertical: 10, paddingHorizontal: 14 },
  viewBtnText:  { color: C.purple, fontWeight: '700', fontSize: 13 },
  ...
  innerTabActive:  { backgroundColor: C.purple },
  innerTabText:    { color: C.muted, fontSize: 13, fontWeight: '600' },
  innerTabTextActive: { color: C.white },

  subCard:     { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface,
                 borderRadius: 12, padding: 14, marginBottom: 10,
                 borderWidth: 1, borderColor: C.border },
  subCardLeft: { flex: 1, gap: 4 },
  subAuthor:   { color: C.text, fontSize: 14, fontWeight: '700' },
  subDesc:     { color: C.muted, fontSize: 12 },
  subVotes:    { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 },
