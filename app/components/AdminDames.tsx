/**
 * AdminDames.tsx — Interface d'administration Dames
 *
 * VUES :
...
interface AdminDamesProps {
  adminEmail:    string;
  adminPassword: string;
  onBack:        () => void;
}

export default function AdminDames({ adminEmail, adminPassword, onBack }: AdminDamesProps) {
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    const res = await fetch(`${BACKEND_URL}/admin-dames`, {
      method: 'POST',
    ...
    <View style={styles.root}>
      ...
      <LinearGradient colors={['#059669', '#047857']} style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>♟️ Admin Dames</Text>
            ...
            <>
              ...
              {showSessionForm && (
                <View style={styles.formBox}>
                  ...
                  <TextInput style={styles.input} value={formTitle}
                    onChangeText={setFormTitle} placeholder="Ex: Tournoi Dames #1" />
              ...
              {sessions.length === 0 ? (
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptyText}>Aucune session Dames</Text>
                </View>
              ) : (
                sessions.map(s => (
              ...
              {matches.length === 0 ? (
                ...
                matches.map((m, i) => (
                  <View key={m.id} style={[styles.matchCard, m.finished && styles.matchCardDone]}>
                    ...
                    {/* Désigner gagnant manuellement si match non terminé */}
                    {!m.finished && selRun?.status === 'launched' && (
                      <View style={styles.winnerActions}>
                        <Text style={styles.winnerActionsLabel}>Désigner le gagnant :</Text>
                        <View style={styles.winnerBtns}>
                          <TouchableOpacity
                            style={styles.winnerBtn}
                            onPress={() => submitMatchResult(m.id, m.player1.id, m.player1.name)}
                            disabled={!!actionLoading}>
                            {actionLoading === `win-${m.id}-${m.player1.id}`
                              ? <ActivityIndicator size="small" color={C.white} />
                              : <Text style={styles.winnerBtnText}>{m.player1.name}</Text>}
                          </TouchableOpacity>
                          <TouchableOpacity
                            style={styles.winnerBtn}
                            onPress={() => submitMatchResult(m.id, m.player2.id, m.player2.name)}
                            disabled={!!actionLoading}>
                            {actionLoading === `win-${m.id}-${m.player2.id}`
                              ? <ActivityIndicator size="small" color={C.white} />
                              : <Text style={styles.winnerBtnText}>{m.player2.name}</Text>}
                          </TouchableOpacity>
                        </View>
                      </View>
                    )}
...
const styles = StyleSheet.create({
  ...
  // Winner actions (admin manual override)
  winnerActions:      { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: C.border },
  winnerActionsLabel: { color: C.muted, fontSize: 12, marginBottom: 8 },
  winnerBtns:         { flexDirection: 'row', gap: 8 },
  winnerBtn:          { flex: 1, alignItems: 'center', justifyContent: 'center',
                        paddingVertical: 10, borderRadius: 10,
                        backgroundColor: C.launched, minHeight: 36 },
  winnerBtnText:      { color: C.white, fontSize: 13, fontWeight: '700' },
