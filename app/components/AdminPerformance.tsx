/**
 * AdminPerformance.tsx — Interface d'administration Performance
 *
 * VUES :
...
export default function AdminPerformance({ adminEmail, adminPassword, onBack }: AdminPerformanceProps) {
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    const res = await fetch(`${BACKEND_URL}/admin-performance`, {
      method:  'POST',
    ...
    <View style={styles.root}>
      ...
      <LinearGradient colors={['#BF360C', '#E65100']} style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>🎭 Admin Performance</Text>
        ...
        <ScrollView style={styles.flex} contentContainerStyle={styles.scroll}>
          ...
          {view === 'sessions' && (
            <>
              ...
              {sessions.length === 0 ? (
                <View style={styles.empty}><Ionicons name="videocam-outline" size={40} color={C.muted} /><Text style={styles.emptyText}>Aucune session Performance</Text></View>
              ) : (
                sessions.map(s => (
