/**
 * AdminArtisanat.tsx — Interface d'administration Artisanat
 *
 * VUES :
...
interface SubmissionGroup {
  ...
  votes: number;
  videos: { id: string; video_url: string; description?: string; duration_s?: number }[];
}
...
interface AdminArtisanatProps {
  adminEmail:    string;
  adminPassword: string;
  onBack:        () => void;
}

export default function AdminArtisanat({ adminEmail, adminPassword, onBack }: AdminArtisanatProps) {
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    const res = await fetch(`${BACKEND_URL}/admin-artisanat`, {
      method:  'POST',
    ...
    <View style={styles.root}>
      ...
      <LinearGradient colors={['#BF360C', '#E65100']} style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>🪡 Admin Artisanat</Text>
        ...
        <ScrollView style={styles.flex} contentContainerStyle={styles.scroll}>
          ...
          {view === 'sessions' && (
            <>
              ...
              {sessions.length === 0 ? (
                <View style={styles.empty}><Ionicons name="videocam-outline" size={40} color={C.muted} /><Text style={styles.emptyText}>Aucune session Artisanat</Text></View>
              ) : (
                sessions.map(s => (
                ...
                runs.map(run => (
                  <View key={run.id} style={styles.runCard}>
                    ...
                    <View style={styles.runMeta}>
                      <MetaCell icon="videocam-outline"  label="Max vidéos" value={String(run.max_submissions)} />
              ...
              {runTab === 'submissions' && (
                <>
                  {submissions.length === 0 ? (
                    <View style={styles.empty}><Ionicons name="videocam-outline" size={40} color={C.muted} /><Text style={styles.emptyText}>Aucune vidéo soumise</Text></View>
                  ) : (
