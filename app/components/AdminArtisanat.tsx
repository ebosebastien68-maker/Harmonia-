 ...
 *
 * CORRECTIFS v6 (vs v5) :
 *   • searchExact → message d'erreur précis selon le cas d'échec
 *     (champ vide, réseau, 0 résultat avec conseil orthographe)
 *   • api() → remonte le champ "details" du backend dans tous les Alert
 *   • pickMsg enrichi : affiche "debug" retourné par le backend si présent
 *   • Validation côté frontend avant d'appeler l'API (évite un aller-retour inutile)
 */
...
function UserSearchExact({
  ...
  return (
    <View>
      ...
      {/* Aide visuelle sur les champs manquants */}
      {(!pickPrenom.trim() || !pickNom.trim()) && pickMsg === null && (
        <Text style={S.searchHint}>
          💡 Saisissez au moins une partie du prénom ET du nom
        </Text>
      )}
...
export default function AdminCommunity({ adminEmail, adminPassword, onBack }: Props) {
  ...
  const api = useCallback(async (body: Record<string, any>) => {
    ...
    try {
      ...
      // Erreur réseau pure (serveur injoignable, timeout, CORS...)
    ...
    if (!res.ok) {
      // On remonte le message précis du backend (error + details si présents)
  ...
  const searchExact = useCallback(async () => {
    // ── Validation frontend avant appel API ──────────────────────────────
    ...
    try {
      ...
      if (users.length === 0) {
        // ── Aucun résultat : message précis avec conseil ─────────────────
        const conseil = d.debug
          // Le backend v3 retourne un champ "debug" explicatif
          ? d.debug
          : `Aucun profil trouvé pour "${prenomV} ${nomV}". Vérifiez l'orthographe et les accents.`;
        setPickMsg({ type: 'err', text: conseil });
      } else {
        setPickMsg({
          type: 'ok',
      ...
      // ── Erreur réseau ou serveur : message complet remontée du backend ──
      setPickMsg({
        type: 'err',
        text: e.message ?? 'Erreur inconnue lors de la recherche.',
      });
    } finally {
  ...
  // Props partagés pour UserSearchExact
  const pickerProps = {
    pickNom, setPickNom,
  ...
  const sendNotification = async () => {
    ...
    if (notifType === 'targeted' && !notifTarget) return Alert.alert('Requis', 'Sélectionnez un utilisateur cible pour une notification ciblée');
...
const S = StyleSheet.create({
  ...
  searchHint: { fontSize: 12, color: C.muted, marginTop: 6, paddingHorizontal: 4, fontStyle: 'italic' },
