// ChatBoxMobile.tsx — v9 Mobile (confirmé fonctionnel iOS & Android)
// Utilisé exclusivement sur Platform.OS !== 'web' via le routeur ChatBox.tsx
// Ne pas importer directement — passer par ChatBox.tsx
//
// Différences clés vs ChatBoxWeb :
//   1. keyboardShouldPersistTaps="handled" → empêche le clavier de voler le tap du bouton Send
//   2. new_message écoute les propres messages (senderId === userId) → libère sending
//   3. Wrappers safeHaptic avec try/catch → anti-crash sur certains appareils
//   4. Failsafe timeout 5s → libère l'UI si le serveur ne répond pas
...
// ─── Wrappers Haptics sécurisés (anti-crash mobile) ──────────────────────────

const safeHapticImpact = (style: Haptics.ImpactFeedbackStyle) => {
  try { Haptics.impactAsync(style); } catch (e) { console.warn('[ChatBoxMobile] Haptics impact failed', e); }
};

const safeHapticNotification = (type: Haptics.NotificationFeedbackType) => {
  try { Haptics.notificationAsync(type); } catch (e) { console.warn('[ChatBoxMobile] Haptics notification failed', e); }
};
...
export default function ChatBoxMobile({
  ...
  // Failsafe : libère l'UI si message_sent ET new_message n'arrivent pas sous 5s
  ...
  const buildPrivateSocket = (): Socket => {
    ...
    socket.on('new_message', (raw: any) => {
      ...
      setMessages(prev => prev.find(m => m.id === msg.id) ? prev : [...prev, msg]);
      // Sur mobile, new_message peut arriver avant ou à la place de message_sent
    ...
    socket.on('error', (err: { message: string }) => {
      console.warn('[ChatBoxMobile] privé erreur:', err.message);
      clearSending();
    });
    return socket;
  };
  ...
  const buildGroupSocket = (): Socket => {
    ...
    socket.on('error', (err: { message: string }) => {
      console.warn('[ChatBoxMobile] groupe erreur:', err.message);
      clearSending();
    });
    return socket;
  };
  ...
  const sendMessage = (mediaUrl?: string, mediaType?: string) => {
    ...
    // Failsafe 5s : si ni message_sent ni new_message n'arrivent → débloque l'UI
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    timeoutRef.current = setTimeout(() => {
      setSending(false);
      timeoutRef.current = null;
      console.warn("[ChatBoxMobile] Délai d'envoi dépassé — déblocage de l'interface.");
    }, 5000);
  ...
  // ── Upload media — fetch/blob (même approche que web, fonctionnel sur mobile) ─
  ...
  const pickAndUploadMedia = async () => {
    try {
      ...
      const urlRes  = await fetch(`${WS_BASE}/media`, {
        ...
        body: JSON.stringify({
          function: 'getUploadUrl', user_id: userId, access_token: accessToken,
          file_name: fileName, file_type: mimeType, context: 'media-premium',
        }),
      });
      ...
      const uploadRes = await fetch(signed_url, {
        method: 'PUT', headers: { 'Content-Type': mimeType }, body: fileBlob,
      });
      if (!uploadRes.ok) {
        setUploading(false);
        alert('Erreur lors de l\'upload vers le stockage');
        console.error('[ChatBoxMobile] PUT erreur:', uploadRes.status);
        return;
      }
      ...
      const confirmRes  = await fetch(`${WS_BASE}/media`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          function: 'confirmUpload', user_id: userId, access_token: accessToken, path, bucket,
        }),
      });
      ...
      setUploading(false);
      console.error('[ChatBoxMobile] upload erreur:', err);
  ...
  const renderBubble = (msg: Message) => {
    ...
    return (
      ...
      >
        ...
        ) : msg.media_url ? renderMediaContent(msg, fromMe) : (
  ...
  return (
    <SafeAreaView style={S.container}>
      ...
      {/* Messages — keyboardShouldPersistTaps="handled" clé mobile */}
      <ScrollView
        ref={scrollRef}
        style={S.msgs}
