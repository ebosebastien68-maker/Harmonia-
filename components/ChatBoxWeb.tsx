// ChatBoxWeb.tsx — Web (comportement v8 d'origine)
// Correction vs v8 : new_message détecte aussi les propres messages → évite le chargement infini
// si message_sent est lent ou ne revient pas (filet de sécurité).
...
export default function ChatBoxWeb({
  ...
  const buildPrivateSocket = (): Socket => {
    ...
    // Confirmation principale : message reçu côté serveur
    socket.on('message_sent', (data: { message: Message }) => {
      addOrIgnore(normalizeMsg(data.message));
      setSending(false);
      scrollToEnd();
    });
    // Filet de sécurité : si message_sent tarde, new_message libère aussi l'UI
    socket.on('new_message', (raw: any) => {
      const msg = normalizeMsg(raw?.message ?? raw);
      addOrIgnore(msg);
      if (msg.senderId === userId) setSending(false); // propre message → libère l'UI
      else onNewMessage?.();
      scrollToEnd();
    });
    ...
    socket.on('error', (err: { message: string }) => {
      console.warn('[ChatBoxWeb] erreur:', err.message);
      setSending(false);
    });
    return socket;
  };
  ...
  const buildGroupSocket = (): Socket => {
    ...
    socket.on('new_message', (raw: any) => {
      const msg = normalizeMsg(raw?.message ?? raw);
      addOrIgnore(msg);
      if (msg.senderId === userId) setSending(false);
      else onNewMessage?.();
      scrollToEnd();
    });
    socket.on('more_messages', (data: { messages: Message[]; has_more: boolean }) => {
  ...
  const loadMore = () => {
    ...
    socketRef.current?.emit('load_more', { [key]: conversationId, user_id: userId, access_token: accessToken, before: messages[0]?.createdAt });
  };
  ...
  const sendMessage = (mediaUrl?: string, mediaType?: string) => {
    ...
    if (conversationType === 'private') {
      ...
      socketRef.current?.emit('send_message', {
        ...
        media_url: mediaUrl ?? null, reply_to_id: reply?.id ?? null,
  ...
  const deleteMessage    = (msg: Message) => socketRef.current?.emit('delete_message',   { message_id: msg.id, user_id: userId, access_token: accessToken });
  ...
  const handleLongPress = (msg: Message) => { setActionMsg(msg); setShowActionSheet(true); };
  const handleTap = (msg: Message) => {
    if (conversationType !== 'group') return;
    const now = Date.now(); const last = lastTapRef.current;
    if (last && last.id === msg.id && now - last.time < 350) { lastTapRef.current = null; if (msg.senderId === userId && msg.reply_to_id) toggleVisibility(msg); }
    else lastTapRef.current = { id: msg.id, time: now };
  };

  // ─── Upload media (fetch/blob — natif web) ───────────────────────────────────
  ...
  const pickAndUploadMedia = async () => {
    try {
      ...
      setUploading(true);
      const urlData = await fetch(`${WS_BASE}/media`, { method:'POST', headers:{'Content-Type':'application/json'},
      ...
      const confirmData = await fetch(`${WS_BASE}/media`, { method:'POST', headers:{'Content-Type':'application/json'},
    ...
    } catch (err) { setUploading(false); console.error('[ChatBoxWeb] upload:', err); alert("Erreur lors de l'upload"); }
  };
  ...
  const renderMedia = (msg: Message, fromMe: boolean) => {
    ...
    if (type === 'image') return (
      <TouchableOpacity activeOpacity={0.9} onPress={() => {
        ...
        if (last && last.url === url && now - last.time < 350) { imgTapRef.current = null; setFullscreenUrl(url); }
    ...
    const iconName = icons[type] ?? 'attach-outline';
    const label    = url.split('/').pop()?.split('?')[0] ?? labels[type] ?? 'Fichier';
    return (
      <View style={S.mediaFileWrap}>
        <Ionicons name={iconName as any} size={22} color={fromMe ? '#fff' : '#8A2BE2'} />
        <Text style={[S.mediaFileName, fromMe ? S.msgTextMe : S.msgTextThem]} numberOfLines={1}>{label}</Text>
      </View>
    );
  };
  ...
  const renderBubble = (msg: Message) => {
    const fromMe = msg.isFromMe; const deleted = !!msg.deletedAt; const isGroup = conversationType === 'group';
    return (
      ...
      >
        ...
        {deleted ? <Text style={[S.msgText, S.msgDeleted]}>Message supprimé</Text>
          ...
          ) : msg.media_url ? renderMedia(msg, fromMe)
  ...
  return (
    <SafeAreaView style={S.container}>
      ...
      <LinearGradient colors={['#8A2BE2','#4B0082']} style={S.header}>
        <TouchableOpacity onPress={onBack} style={S.backBtn}><Ionicons name="arrow-back" size={24} color="#fff" /></TouchableOpacity>
        {conversationType === 'private' ? (
          ...
          <View style={{ flexDirection:'row', alignItems:'center', flex:1 }}>
            <View style={[S.avatar, { backgroundColor:'#FF8C00' }]}><Ionicons name="people" size={20} color="#fff" /></View>
            <View style={S.headerInfo}>
      ...
      <ScrollView ref={scrollRef} style={S.msgs} contentContainerStyle={{ padding:15, paddingBottom:20 }}>
        {hasMore && !loading && (
          <TouchableOpacity onPress={loadMore} disabled={loadingMore} style={S.loadMoreBtn}>
            {loadingMore ? <Text style={S.loadMoreTxt}>Chargement…</Text>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={S.inputWrap}>
          {conversationType === 'group' && currentRole === 'userpro' && (
            <TouchableOpacity style={S.mediaBtn} onPress={pickAndUploadMedia} disabled={uploading || sending} activeOpacity={0.7}>
          ...
          <TextInput style={S.input} placeholder="Écrivez un message..." placeholderTextColor="#999"
            value={input} onChangeText={handleTyping} multiline maxLength={1000} editable={!sending && !uploading} />
          <TouchableOpacity style={[S.sendBtn, (!input.trim() || sending) && S.sendBtnDisabled]}
            onPress={() => sendMessage()} disabled={!input.trim() || sending} activeOpacity={0.7}>
            ...
            {actionMsg && (<>
              {conversationType === 'group' && !actionMsg.deletedAt && !actionMsg.is_private_reply && (<>
                <TouchableOpacity style={S.actionItem} onPress={() => { setReplyingTo(actionMsg); setReplyVisible(true); setShowActionSheet(false); }}>
                  ...
                  <View style={{ flex:1 }}><Text style={[S.actionTxt, { color:'#8A2BE2' }]}>Répondre en public</Text><Text style={S.actionSubTxt}>Tout le groupe voit la réponse</Text></View>
                </TouchableOpacity>
                <TouchableOpacity style={S.actionItem} onPress={() => { setReplyingTo(actionMsg); setReplyVisible(false); setShowActionSheet(false); }}>
                  ...
                  <View style={{ flex:1 }}><Text style={[S.actionTxt, { color:'#6B21A8' }]}>Répondre en privé</Text><Text style={S.actionSubTxt}>Visible uniquement par vous et le destinataire</Text></View>
                </TouchableOpacity>
              </>)}
...
const S = StyleSheet.create({
  ...
  header:{ flexDirection:'row', alignItems:'center', paddingTop:10, paddingBottom:15, paddingHorizontal:15 },
