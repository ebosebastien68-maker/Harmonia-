// ChatBoxWeb.tsx — v8 Web (stable, comportement d'origine)
...
export default function ChatBoxWeb({
  ...
  const buildPrivateSocket = (): Socket => {
    ...
    socket.on('error', (err: { message: string }) => {
      console.warn('[ChatBoxWeb] privé erreur:', err.message);
      setSending(false);
    });
    return socket;
  };
  ...
  const buildGroupSocket = (): Socket => {
    ...
    socket.on('error', (err: { message: string }) => {
      console.warn('[ChatBoxWeb] groupe erreur:', err.message);
      setSending(false);
    });
    return socket;
  };
  ...
  // ── Upload media — Signed URL (fetch/blob — web natif) ───────────────────────
  ...
  const pickAndUploadMedia = async () => {
    try {
      ...
      if (!uploadRes.ok) {
        ...
        console.error('[ChatBoxWeb] PUT erreur:', uploadRes.status, await uploadRes.text());
        return;
      }
      ...
      setUploading(false);
      console.error('[ChatBoxWeb] upload erreur:', err);
